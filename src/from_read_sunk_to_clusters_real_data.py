...
def get_pairwise_location_data(pw_profiles, threshold):
    overlap_locations = [["r1", "r2", "sunk_delta_mean", "sunk_delta_sd", "#_of_SUNKs"]]
    read_set = set()
    for key in pw_profiles.keys():
        ...
        if len(shared_sunks) > threshold:
            read_set.add(r1)
            read_set.add(r2)
            overlap_locations.append([r1, r2, np.round(sunk_delta_mean, 2), np.round(sunk_delta_sd, 6), no_shared_sunk])
...
def get_clusters_from_sunks_and_reads(sunk_file, read_file1, read_file2, input_filename, method):
    sunk_map = get_sunk_map(sunk_file)
    out_sunk_map = inters_path+input_filename+"/sunk_map.txt"
    write_sunk_map(out_sunk_map, sunk_map)
    r2s, s2r = get_read_to_sunk_and_sunk_to_read_maps(sunk_map, read_file1)
    out_r2s = inters_path+input_filename+"/read_to_sunk_map.txt"
    out_s2r = inters_path+input_filename+"/sunk_to_read_map.txt"
    ...
    out_pw_profiles = inters_path+input_filename+"/pairwise_profiles.txt"
    write_pairwise_profiles(pw_profiles, out_pw_profiles)
    print("--- pairwise profiling done ---")
    #in_maf = reads_path + input_filename + ".maf"
    #maf_data = maf_parser(in_maf)
    #out_alignment = inters_path+input_filename+"/alignment.csv"
    ...
    out_overlap = inters_path+input_filename+"/overlap.csv"
...
#    print("# of reads:", len(reads))
    out_path = clusters_path + input_filename + "/" + method + "/"
    if method == "naive":
        greedy = False
    ...
    out_clusters = clusters_path + input_filename + "/" + method + "/"
...
methods = ["naive", "cc"]
reads_path = "../arabidopsis/"
#sunks_path = "../kmers/"
inters_path = "../arabidopsis/intermediates/"
clusters_path = "../arabidopsis/clusters/"
...
#            for depth in depths:
input_filename = "err.chr5_11300k-11350k"
print("Start:", input_filename)
sunk_file = open(reads_path + input_filename + ".kmers")
read_file1 = SeqIO.parse(reads_path + input_filename + ".fasta", "fasta")
read_file2 = SeqIO.parse(reads_path + input_filename + ".fasta", "fasta")
#ref_size = 100000 + (copy * repeat_size)
get_clusters_from_sunks_and_reads(sunk_file, read_file1, read_file2, input_filename, methods[0])
print("Done:", input_filename)